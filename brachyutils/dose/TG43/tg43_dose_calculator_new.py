...
from brachyutils.unit_utils import CM, U, DEG, GY, S, MM

class BrachyUtilsTG43New(BrachyDoseGenerator):
    """
    """
    def __init__(self,
        dir_tg43_parameters: Optional[Union[Path, str]] = Path(),
        ...
        **calc_parameter_kwargs
        ...
        self.calc_parameters = {
            ...
            "kernel_max_radius" : None, #if None, calculated automatically
            "kernel_radial_resolution" : 0.005 * CM,
            "kernel_angular_resolution" : 0.5 * DEG
        }
    ...
    def load_from_brachyplan(self, plan : BrachyPlan) -> None:
        ...
        if self.dir_tg43_parameters == Path():
            self.dir_tg43_parameters = (
                Path(__file__).parent.parent.parent
                / "admin/constants/TG43_Parameter_Data"
                / self.source_name
                )
        self.is_hdr : bool = self.brachysource.treatment_type == "HDR"
        self.air_kerma_strength = self.brachysource.air_kerma_strength * U
    ...
    def auto_calculate_phantom(self) -> BrachyPhantom:
        ...
        new_phantom = self.brachyplan.phantom.crop_by_contour(contour_name = structures_to_crop_by, inplace = False, marginInMM = 20.0 * MM)
        new_phantom.cached_structure_masks = None #a little hack to prevent resampling of structure masks (useless)
        new_phantom.resample_to(spacing=np.array([1.0 * MM , 1.0 * MM , 1.0 * MM]), inplace=True)
