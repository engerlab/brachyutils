...
class Config_Catheter_Rotation(BaseModel):
    ...
    ### Attributes:
    - `x_angle_max`  : half-wdith angle away from normal rotating around y axis (degrees);
    ...
    - `y_angle_max`   : half-width angle away from normal rotating around x axis (degrees);
    sweeps -y_angle_max to +y_angle_max.
    - `y_angle_step`  : ap angle increment (degrees)
    """
    x_angle_max: float = Field(default=10, description="  half-wdith angle away from normal rotating around y axis (degrees);\
sweeps -x_angle_max to +x_angle_max. 0 means parallel to normal, 90 means perpendicular to normal.")
    x_angle_step: float = Field(default=10, description="right-left angle increment (degrees)")
    y_angle_max: float = Field(default=10, description="half-width angle away from normal rotating around x axis (degrees); \
sweeps -y_angle_max to +y_angle_max.")
...
catheter segments and organs at risk (OARs) (mm).")
    segment_collision_margin_mm: float = Field(default=5.1, description="the collision margin between \
