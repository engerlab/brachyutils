...
class GVarianJaws(BeamModel):
    """Generic Varian beam model."""

    model_name = "GVarianJaws"
    folder = "BEAM_GVarianJaws"
    pegs_file = "radify"
    particle = "photon"
    ...
    def make_sim_inputs(self, simulation):
        ...
        # Eww, but no choice because BeamNRC class looks at this attribute
        self.folder = "BEAM_{model_name}".format(model_name=self.model_name, mlc_type=params["mlc_type"])
        ...
        params["model_path"] = beam_model_path
        ...
        params["filler_depth"] = simulation.settings.get("filler_depth", 40.0)
    ...
    def make_aperture_inputs(self, beamlet_creation):
        ...
        self.folder = "BEAM_{model_name}_{mlc_type}".format(model_name=self.model_name, mlc_type=params["mlc_type"])
        beam_model_path = os.path.join(beamnrc_path, self.folder)
        dosxyznrc_path = os.path.join(beamnrc_path, "dosxyznrc")
