...
from pyRad.SimCodes.BeamNRC.BeamModels.GenericVarian import GenericVarian

class GVarianStat(GenericVarian.GenericVarian):
    """
    Generic Varian statistical beam model. Inherit everything from GenericVarian
    and only redefine the input file writing routines.
    """

    model_name = "GVarianStat"
    folder = "BEAM_GVarianStat"
    pegs_file = "radify"
    particle = "photon"
    ...
    def _get_stats_model(self, params):
        ...
        """
        mcmodel = params.get("mcmodel", "GVarianStat")
        ident = params["energy"]
        ...
        return "{mcmodel}_{ident}_{energy}_{fwhm}.mcmodel".format(mcmodel=mcmodel, ident=ident, energy=e_energy, fwhm=e_fwhm)
