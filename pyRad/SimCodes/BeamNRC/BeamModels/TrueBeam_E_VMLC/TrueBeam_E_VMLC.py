"""
TrueBeam electron beam model module.
...
class TrueBeam_E_VMLC(BeamModel):
    """TrueBeam electron model with Millenium MLC."""

    model_name = "TrueBeam_E_VMLC"
    folder = "BEAM_TrueBeam_E"
    pegs_file = "radify"
    particle = "electron"

    # New calibration factors including k_ecal
    calibration_factors = {
        "20": 1.9758765778401122e-11,
        "16": 1.7540685944855413e-11,
        "12": 1.6325428194993413e-11,
        "9": 1.3138594164456234e-11,
        "6": 7.91588785046729e-12
    }
    ...
    def make_sim_inputs(self, simulation):
        ...
        energy = cpts[0].get("energy", 6)
        phsp_name = "TrueBeam_{energy}E.IAEAphsp".format(energy=int(energy))
    ...
    def make_aperture_inputs(self, beamlet_creation):
        ...
        for cpt_index, cpts in enumerate(scenario_cpts):
            ...
            for sc_index, cpt in enumerate(cpts):
                ...
                phsp_name = "TrueBeam_{energy}E.IAEAphsp".format(energy=energy)
    ...
    def generate_cpt_beamlets(self, beamlet_creation):
        ...
        for cpt in beamlet_creation.control_points:
            ...
            for b_index, included in enumerate(beamlet_mask):
                if included:
                    files = self._make_beamlet_input(b_index, cpt, beamlet_creation, scenarios)
                    files_created += files
    ...
    def _make_beamlet_input(self, beamlet_index, cpt, beamlet_creation, scenarios=None):
        ...
        phsp_file = "TrueBeam_{energy}E_beamlets_{index}.egsphsp1".format(energy=int(cpt.energy), index=beamlet_index)
        ...
        for sc_i, shift in enumerate(scenarios):
            params["iso_x"] = (cpt.iso[0] + shift[0]) / 10.0
            params["iso_y"] = (cpt.iso[1] + shift[1]) / 10.0
            params["iso_z"] = (cpt.iso[2] + shift[2]) / 10.0
            filename = "{name}_{cpt_index}_{beamlet_index}_{energy}MeV_{scenario_index}.egsinp".format(
                name=beamlet_creation.settings["name"],
                cpt_index=cpt.index,
